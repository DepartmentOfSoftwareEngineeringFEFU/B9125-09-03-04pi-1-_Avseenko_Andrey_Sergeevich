...
void DeleteElement(Node*& head, Node*& tail, int index, int length){
    ...
    if(index < 0 || index >= length){std::cout<<"Неверный индекс! Искомый элемент за пределами списка"; return;}
    if (head == nullptr){
        std::cout<<"Списка не существует!\n";
        return;
    }

    if(length == 1){ //на всякий случай обработка длины 1?
        delete head;
        head = nullptr;
        tail = nullptr;
        return;
    ...
    if(index == length-1){ //не уверен что правильно кидать сюда длину -1 
        Node* temp = tail;
        tail = tail->prev;
        tail->next = nullptr;
        delete temp;
    ...
    if(index <= length / 2){
        ...
        while(current != nullptr && tempIndex < index){
            current = current->next;
            tempIndex ++;
        }
        ...
        current = tail;
        int tempIndex = length-1; // length = tail в этой реализации вроде
        ...
        toDelete->next->prev = toDelete->prev; //теперь вроде хорошо
        delete toDelete;
}
